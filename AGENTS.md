...
- **Brush presets**: `state.brushSettings` keeps per-mode brush size/hardness and fixed-feather pixel values (erase defaults to 10% size, repair defaults to 5% with shared hardness) so swapping modes restores each brush's last settings; `state.isFeatherFixed` toggles hardness vs fixed-pixel feathering.
