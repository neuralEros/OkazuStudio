...
- **Brush presets**: `state.brushSettings` keeps per-mode brush size plus both hardness-percent and fixed-feather pixel values (erase defaults to 10% size, repair defaults to 5% with shared feather defaults) so swapping modes restores each brush's last settings while the global feather toggle flips between percent hardness and fixed pixel feather.
