...
- **Brush presets**: `state.brushSettings` keeps per-mode brush size plus both relative hardness and fixed-pixel feather values (erase defaults to 10% size, repair defaults to 5% with shared defaults), so swapping modes restores each brush's last settings.
