...
- **Testability hooks**: `window.OkazuTestables` exposes pure helpers from input/adjustments/stego/kakushi/watermark/main for offline unit tests, and `scripts/test_runner.js` + `scripts/tests.js` register and execute in-app checks via the Debug panel.
