...
- **Adjusted layer copies**: each loaded image has a working canvas (`state.workingA`/`state.workingB`) that bakes current adjustments so regular renders can composite without per-frame pixel filtering. Working copies are rebuilt when sliders finish changing or when images are replaced, and a version counter keeps baked canvases in sync with the latest adjustments so renders never recompute filters every frame.
