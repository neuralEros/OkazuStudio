...
- **Canvas stack**: main display canvas (`#mainCanvas`) sits inside a transformable wrapper (`#canvas-wrapper`) controlled by the viewport for pan/zoom. Offscreen canvases include `maskCanvas` (brush strokes), `frontLayerCanvas` (front image after mask), and preview canvases for throttled adjustment previews. A visible fast preview surface (`#fastPreviewCanvas`) overlays the main canvas during strokes and scales a 1080p composite via CSS so the full-resolution canvas stays untouched while drawing.
