1. Scope and Environment Setup
    [ ] 1.1. Define test scope for scripts/kakushi.js
    ...
    [ ] 1.3. Implement Test Helpers
        [ ] 1.3.1. makeCanvas(w, h, fillRGBA)
            # Returns HTMLCanvasElement
        [ ] 1.3.2. setPixel(ctx, x, y, [r,g,b,a])
            # Writes a single pixel
        [ ] 1.3.3. getPixel(ctx, x, y)
            # Reads pixel
        [ ] 1.3.4. rgbaIndex(x, y, w)
            # formula: ((y * w) + x) * 4
        [ ] 1.3.5. asImageData(canvas)
            # ctx.getImageData(0,0,w,h)
    [ ] 1.4. Define Protocol Constants
...
2. Shared Fixtures
    [ ] 2.1. Opaque base canvas
        # 4x4, every pixel [10, 20, 30, 255]
    [ ] 2.2. Transparent pixel canvas
        # 4x4, all opaque except (0,0) alpha 0
    [ ] 2.3. Mask data
        # ImageData-like array with same length as imageData.data
        [ ] 2.3.1. maskNone: all zeros (no masking)
        [ ] 2.3.2. maskAt00: same size, alpha 255 at (0,0) only, rest 0
    [ ] 2.4. Payload fixtures
        [ ] 2.4.1. payloadSmall: new Uint8Array([0b10101010, 0b11001100])
        [ ] 2.4.2. payloadEmpty: new Uint8Array([])

3. peek Tests
    [ ] 3.1. Header detection (magic present, opaque, no mask)
    ...
    [ ] 3.3. Mask filtering skips masked pixels
        ...
        # Assertions: expect(result).toBe(false)
    [ ] 3.4. Error handling when context unavailable
...
4. seal Tests
    [ ] 4.1. Capacity calculation excludes transparent and masked pixels
    ...
    [ ] 4.2. Payload length check uses compressed length
    ...
    [ ] 4.5. Throws on insufficient capacity
    ...
    [ ] 4.6. Final canvas output contains embedded payload
        # Setup: 2x2 opaque canvas.
...
5. reveal Tests
    ...
    [ ] 5.6. Decompression failure path sets error
...
6. Helper Tests
    [ ] 6.1. getContextForReading with canvas/context/source
    ...
    [ ] 6.2. normalizeMask/isMasked behavior
    ...
    [ ] 6.3. embedBytes/extractBytes round-trip
    ...
    [ ] 6.4. sanitizeRegion bit count accuracy
