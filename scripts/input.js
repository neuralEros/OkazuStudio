function createInputSystem({ state, els, maskCtx, maskCanvas, render, saveSnapshot, undo, redo, showHints, scheduleHeavyTask, acceptCrop, cancelCrop, setBrushMode }) {
    ...
    function getCropMinScale() {
        ...
        const bounds = state.cropRotation ? window.Geometry.getRotatedAABB(visualW, visualH, state.cropRotation) : { w: visualW, h: visualH };
    ...
    function getBrushPixelSize() {
        ...
        return window.Geometry.toPixels(state.brushSize, state.fullDims.h);
    }
    ...
    function getCanvasCoordinates(e) {
        ...
        if (!state.isCropping && state.cropRotation !== 0 && state.cropRect) {
            ...
            const rotated = window.Geometry.rotatePoint({ x: canvasX, y: canvasY }, visualW / 2, visualH / 2, -state.cropRotation);
            canvasX = rotated.x;
            canvasY = rotated.y;
        }
        ...
        if (!state.isCropping && state.cropRect) {
            ...
            const localP = window.Geometry.visualToTruthCoords(canvasX, canvasY, state.rotation, localTruthW, localTruthH);
        ...
        const absP = window.Geometry.visualToTruthCoords(canvasX, canvasY, state.rotation, state.fullDims.w || 1, state.fullDims.h || 1);
    ...
    function getCropPivot(e) {
        ...
        const unrotated = window.Geometry.rotatePoint({x: canvasX, y: canvasY}, canvasCx, canvasCy, -state.cropRotation);

        // 3. Convert to Truth
        const absP = window.Geometry.visualToTruthCoords(unrotated.x, unrotated.y, state.rotation, fullW, fullH);
        return { x: absP.x / fullH, y: absP.y / fullH };
    }
    ...
    function enforceCropView(maintainScale = false, recenter = false) {
        ...
        if (state.cropRotation !== 0) {
            ...
            const rotatedCenter = window.Geometry.rotatePoint({x: cropCx, y: cropCy}, canvasCx, canvasCy, state.cropRotation);
    ...
    function handlePointerMove(e) {
        ...
        if (state.isPanning) {
            ...
            if (state.cropDrag.type === 'rotate') {
                ...
                const baseVisual = window.Geometry.truthToVisualRect(startTruthPx, state.rotation, fullW, fullH);
                ...
                // Back to Truth
                const newTruth = window.Geometry.visualToTruthRect(baseVisual, state.rotation, fullW, fullH);
                ...
                const baseC = window.Geometry.rotatePoint({x: anchorX, y: anchorY}, canvasCx, canvasCy, -startRotation);
                const newBaseVisual = {
                ...
                const newTruth = window.Geometry.visualToTruthRect(newBaseVisual, state.rotation, fullW, fullH);
                state.cropRect = {
                    x: newTruth.x / fullH,
                    y: newTruth.y / fullH,
                    w: newTruth.w / fullH,
                ...
                const baseC = window.Geometry.rotatePoint({x: vCx, y: vCy}, canvasCx, canvasCy, -startRotation); // Use Start Rotation
    ...
    function handleWheel(e) {
        ...
        if (e.ctrlKey || e.metaKey) {
            const delta = -Math.sign(e.deltaY) * 0.002;
    ...
    function getVisualStartRect() {
        ...
        const baseVisual = window.Geometry.truthToVisualRect(truthRectPx, state.rotation, fullW, fullH);
        ...
        const newCenter = window.Geometry.rotatePoint({x: cx, y: cy}, canvasCx, canvasCy, state.cropRotation);
    ...
    function attachInputHandlers() {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === '[' || e.key === ']') {
                const step = e.repeat ? 0.004 : 0.002;
