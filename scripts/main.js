             ...
             try {
                 ...
                 if (!isStego && window.kakushi) {
                     ...
                     if (n.includes('_save') || n.includes('_project') || n.includes('okazustudio')) {
                         ...
                         Logger.warn("Stego header missing but filename suggests save file. Prompting for forced reveal...");
                         ...
                         if (confirm) {
                             try {
                                 ...
                                 if (result.secret) {
                                     isStego = true;
                                     Logger.info("Forced reveal successful.");
                                     ...
                                     if (result2.secret) {
                                         isStego = true;
                                         cleanImg = await loadImageSource(tempCanvas.toDataURL());
                                         Logger.info("Forced reveal (after unwatermark) successful.");
                                     } else {
                                         Logger.warn("Forced reveal failed even after watermark removal.");
        ...
        function loadImageSource(source) {
            return new Promise(async (resolve, reject) => {
                if (source instanceof Blob) {
                    // Try createImageBitmap for color fidelity (Chromium fix)
                    if (window.createImageBitmap) {
                        try {
                            const bitmap = await createImageBitmap(source, { colorSpaceConversion: 'none' });
                            // Convert Bitmap to Canvas to ensure downstream compatibility (ImageBitmap is not a DOM Node)
                            const canvas = document.createElement('canvas');
                            ...
                            Logger.warn("createImageBitmap failed, falling back to Image", e);
                        }
                    }
