(function() {
    const { register, assert, assertEqual, assertDeepEqual } = window.TestRunner;
    const {
        buildMaskCanvas, rotatePoint, apply, buildMask, checkAndRemove
    } = window.OkazuTestables.watermark;

    // 1. Helpers
    function makeCanvas(w, h, fill = [0,0,0,0]) {
        const c = document.createElement('canvas');
        c.width = w; c.height = h;
        const ctx = c.getContext('2d');
        ctx.fillStyle = `rgba(${fill[0]},${fill[1]},${fill[2]},${fill[3]/255})`;
        ctx.fillRect(0,0,w,h);
        return c;
    }

    function getPixel(ctx, x, y) {
        return ctx.getImageData(x, y, 1, 1).data;
    }

    function isOpaque(pixel) {
        return pixel[3] === 255;
    }

    function isTransparent(pixel) {
        return pixel[3] === 0;
    }

    // 2. Shared Fixtures
    const MASK_WIDTH = 100;
    const MASK_HEIGHT = 60;
    const MASK_SAMPLES = [
        { x: 47, y: 0, alpha: 255 },
        { x: 50, y: 0, alpha: 255 },
        { x: 15, y: 30, alpha: 255 },
        { x: 66, y: 30, alpha: 255 },
        { x: 5, y: 5, alpha: 0 },
        { x: 90, y: 10, alpha: 0 },
        { x: 10, y: 50, alpha: 0 },
        { x: 80, y: 45, alpha: 0 }
    ];

    // 3. rotatePoint Tests
    register('Watermark: rotatePoint', () => {
        // 3.1 0 deg
        const p0 = rotatePoint(10, 15, 0, 0, 0, 1);
        assertDeepEqual(p0, { x: 10, y: 15 });

        // 3.2 90 deg around (5,5)
        // Point (6,5). dx=1, dy=0.
        // sin=1, cos=0.
        // rx = 1*0 - 0*1 = 0.
        // ry = 1*1 + 0*0 = 1.
        // x = 0+5=5. y=1+5=6.
        const p90 = rotatePoint(6, 5, 5, 5, 1, 0);
        assertDeepEqual(p90, { x: 5, y: 6 });

        // 3.3 180 deg around (10,10)
        // Point (12,9). dx=2, dy=-1.
        // sin=0, cos=-1.
        // rx = 2*-1 - -1*0 = -2.
        // ry = 2*0 + -1*-1 = 1.
        // x = -2+10=8. y=1+10=11.
        const p180 = rotatePoint(12, 9, 10, 10, 0, -1);
        assertDeepEqual(p180, { x: 8, y: 11 });
    });

    // 4. buildMaskCanvas Tests
    register('Watermark: buildMaskCanvas', () => {
        // 4.1 Size
        const cvs = buildMaskCanvas(MASK_WIDTH, MASK_HEIGHT);
        assertEqual(cvs.width, MASK_WIDTH);
        assertEqual(cvs.height, MASK_HEIGHT);

        // 4.3 Binary Alpha
        const ctx = cvs.getContext('2d');
        const data = ctx.getImageData(0, 0, MASK_WIDTH, MASK_HEIGHT).data;
        let hasOpaque = false;
        let hasTransparent = false;
        for (let i=3; i<data.length; i+=4) {
            if (data[i] === 255) hasOpaque = true;
            else if (data[i] === 0) hasTransparent = true;
            else throw new Error(`Non-binary alpha: ${data[i]}`);
        }
        assert(hasOpaque, 'Has content');
        assert(hasTransparent, 'Has holes');

        // 4.4 Deterministic coordinate sampling
        MASK_SAMPLES.forEach((sample) => {
            const pixel = getPixel(ctx, sample.x, sample.y);
            assertEqual(pixel[3], sample.alpha, `Alpha at (${sample.x},${sample.y})`);
        });

        // 4.5 Quadrant coverage
        const quadrantCounts = [0, 0, 0, 0];
        for (let y = 0; y < MASK_HEIGHT; y++) {
            for (let x = 0; x < MASK_WIDTH; x++) {
                const alpha = data[(y * MASK_WIDTH + x) * 4 + 3];
                if (alpha !== 255) continue;
                const quadrant = (y < MASK_HEIGHT / 2 ? 0 : 2) + (x < MASK_WIDTH / 2 ? 0 : 1);
                quadrantCounts[quadrant] += 1;
            }
        }
        quadrantCounts.forEach((count, idx) => {
            assert(count > 0, `Quadrant ${idx} has opaque pixels`);
        });
    });

    // 5. buildMask Tests
    register('Watermark: buildMask', () => {
        const mask = buildMask(MASK_WIDTH, MASK_HEIGHT);
        assertEqual(mask.width, MASK_WIDTH);
        assertEqual(mask.height, MASK_HEIGHT);

        let hasOpaque = false;
        let hasTransparent = false;
        for (let i = 3; i < mask.data.length; i += 4) {
            const alpha = mask.data[i];
            if (alpha === 255) hasOpaque = true;
            else if (alpha === 0) hasTransparent = true;
            else throw new Error(`Non-binary alpha: ${alpha}`);
        }
        assert(hasOpaque, 'Mask has opaque pixels');
        assert(hasTransparent, 'Mask has transparent pixels');

        MASK_SAMPLES.forEach((sample) => {
            const idx = (sample.y * MASK_WIDTH + sample.x) * 4 + 3;
            assertEqual(mask.data[idx], sample.alpha, `Mask alpha at (${sample.x},${sample.y})`);
        });
    });

    // 5. Apply / Remove Tests
    register('Watermark: Application Logic', () => {
        const w = 20, h = 20;
        const cvs = makeCanvas(w, h, [10, 10, 10, 255]);
        const ctx = cvs.getContext('2d');

        // Capture original
        const orig = ctx.getImageData(0,0,w,h).data;
        const origCopy = new Uint8ClampedArray(orig);

        // 5.1 Apply
        apply(ctx, w, h);
        const watermarked = ctx.getImageData(0,0,w,h).data;

        // Should differ
        let diff = false;
        for(let i=0; i<orig.length; i++) if (watermarked[i] !== orig[i]) diff = true;
        assert(diff, 'Watermark applied changes');

        // 5.2 Remove (Idempotent / XOR)
        // checkAndRemove applies it again (XOR)
        checkAndRemove(cvs);
        const restored = ctx.getImageData(0,0,w,h).data;

        // Verify byte-for-byte equality
        for(let i=0; i<orig.length; i++) {
            if (restored[i] !== origCopy[i]) {
                throw new Error(`Mismatch at ${i}: ${restored[i]} vs ${origCopy[i]}`);
            }
        }
        assert(true, 'Restored perfectly');

        // 5.3 Idempotent after multiple passes
        checkAndRemove(cvs);
        const restoredAgain = ctx.getImageData(0,0,w,h).data;
        for(let i=0; i<orig.length; i++) {
            if (restoredAgain[i] !== origCopy[i]) {
                throw new Error(`Mismatch after second removal at ${i}: ${restoredAgain[i]} vs ${origCopy[i]}`);
            }
        }
        assert(true, 'Restored perfectly after second removal');
    });

})();
